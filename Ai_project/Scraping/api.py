...
# Custom JSON encoder to handle NaN values and ObjectId
...
def is_duplicate_job(job1, job2, title_threshold=0.8, company_threshold=0.7):
    """
    ...
    # If both title and company are very similar, consider them duplicates
    return (title_ratio >= title_threshold and company_ratio >= company_threshold) or \
           (title_ratio >= 0.9 and same_location)  # Very similar title and same location
...
def search_jobs():
    ...
    try:
        ...
        if remove_duplicates:
            ...
            results = remove_duplicate_jobs(results)
        ...
        return jsonify({'jobs': formatted_results})
...
def get_job_details(job_id):
    ...
    try:
        ...
        return jsonify({'job': formatted_job, 'similarJobs': formatted_similar_jobs})
...
def initialize_vector_store(data_dir, vector_dir):
    ...
    try:
        ...
        for i, job in enumerate(vector_store.jobs):
            if 'id' not in job:
                job['id'] = f"job-{i+1}"
